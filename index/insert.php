...
if(isset($_GET["valid"])){
  if($_GET["valid"] == 1){
    ...
    $statement = $pdo->prepare("INSERT INTO `termine` (title, note, location, full_day, start_time, end_time, edit_allowed_for, read_allowed_for, color, created_by_id, created_at) VALUES (:title, :note, :location, :full_day, :start_time, :end_time, :edit_allowed_for, :read_allowed_for, :color, :created_by_id, :created_at)");
    $statement->execute(array('title'=> $_POST["title"], 'note'=> $_POST["note"], 'location'=> $_POST["location"], 'full_day'=> $full_day, 'start_time'=> $start_time, 'end_time'=> $end_time, 'edit_allowed_for' => $edit_allowed_for, 'read_allowed_for' => $read_allowed_for, 'color' => $_POST["color"], 'created_by_id' => $_SESSION["userid"], 'created_at' => date("Y-m-d H:i:s")));
    if(!empty($_FILES['file']['name'])){
      $upload_folder = "../meta/pictures/terminpic/terminid".$pdo->lastInsertId()."/";; //Das Upload-Verzeichnis
    ...
    //include_once '../meta/checkcollision/checkcollision.php?id='.$pdo->lastInsertId();
    //die('<meta http-equiv="refresh" content="0; URL=index.php?id='.$pdo->lastInsertId().'">');
  }
}
?>
<!DOCTYPE html>
<html>
  <title>Hinzugefügt :: Kalender :: SyCa</title>
  <?php
    ...
    <div class="limiter limiter-show">
      <div class="container-login100">
        <div class="wrap-login100">
          <span class="login100-form-title" style="color: #72af2f;">Neuer Termin hinzugefügt</span>
